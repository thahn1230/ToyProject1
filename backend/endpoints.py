...
def generate_response(params: dict):
    response = openai.ChatCompletion.create(
        ...
        messages=[
            ...
            {
                "role": "system",
                "content": "List of data is as follows : " + test_data
            },
            {
                "role": "system",
                "content": "Time now is : " + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " so your answer must include open restaurants."
            },
            {
                "role": "user",
...
def test_response():
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            ...
            {
                "role": "user",
                "content": "recommend me some japanese food."
            }
        ],
        temperature=0.2
    )
